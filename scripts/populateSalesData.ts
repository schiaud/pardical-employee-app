// Script to populate sales subcollection for testing
// Run with: npx ts-node scripts/populateSalesData.ts
...
const firebaseConfig = {
  ...
  storageBucket: "pardical-web-app.appspot.com",
...
// Normalize item name (same logic as in Cloud Functions)
...
async function populateSalesData() {
  const itemName = "94-99 Toyota Celica GT ST ST204 AT200 OEM Gas Fuel Pump Sending Assembly";
  const normalizedId = normalizeItemName(itemName);

  console.log(`Populating sales for item: ${itemName}`);
  ...
  const itemStatsRef = doc(db, 'itemStats', normalizedId);
  ...
  for (let i = 0; i < sales.length; i++) {
    ...
    const saleRef = doc(collection(itemStatsRef, 'sales'), saleId);
    ...
    console.log(`Created sale ${i + 1}: ${sale.date} - $${sale.price} (profit: $${profit.toFixed(2)})`);
  }

  console.log('\nDone! Created 21 sales records.');
}

populateSalesData().catch(console.error);
